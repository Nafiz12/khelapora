...
class Ac_receipt_vouchers extends MY_Controller {
    ...
    function index() {
        ...
        $total = $this->Ac_voucher->row_count($cond,'R');

        //Initializing Pagination

        $config['enable_query_string']=TRUE;
        $config['suffix']="?".  http_build_query($session_data,"&amp;");
        $config['base_url'] = site_url('/ac_receipt_vouchers/index');
        ...
        $data['list'] = $this->Ac_voucher->get_list($config['per_page'], (int) $this->uri->segment(3),$cond,'R');
        $data['session_data'] = $session_data;
        $data['headline'] = 'List of Receipt Voucher';
        $data['title'] = 'Receipt Voucher Lists';
        $this->layout->view('ac_receipt_vouchers/index',$data);
    }
    ...
    function add() {
        ...
        if ($_POST) {
            ...
            if(!isset($data['CreditLedgerId']) || empty($data['CreditLedgerId']) || empty($data['Amount'])){
                ...
                redirect('ac_receipt_vouchers/index', 'refresh');
            }
        ...
        $data = $this->_load_combo_data();
        $data['VoucherType'] = 'R';
        $data['headline'] = 'Add Receipt Voucher';
        $data['title'] = 'Add Receipt Voucher';
        $this->layout->view('ac_receipt_vouchers/save', $data);
    }
    ...
    function edit($id = null) {
        ...
        $data = $this->_load_combo_data();
        $data['headline'] = 'Edit Receipt Voucher';
    ...
    function view($id = null) {
        $data['headline'] = 'View Receipt Voucher';
        ...
        if (empty($id)) {
            ...
            $branch_info = $this->Branch->read($data['voucher_info']->BranchId);
            $data['voucher_details_data'] = $this->Ac_voucher->get_voucher_details_data_for_receipt($id);
            $data['branch_info'] = $branch_info;
            $this->layout->view('ac_receipt_vouchers/view', $data);
        }
    }
